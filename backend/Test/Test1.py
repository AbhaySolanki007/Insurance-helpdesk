...
FAQ_DB_PATH = os.getenv(
    "FAQ_DB_PATH",
    r"D:\cywarden\project2\Insurance-Helpdesk\backend\backend-python\Unified Bot\Unified Bot\faq_database",
)
TICKET_DB_PATH = os.getenv(
    "TICKET_DB_PATH",
    r"D:\cywarden\project2\Insurance-Helpdesk\backend\backend-python\Unified Bot\Unified Bot\ticket_database",
)
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                "secret.json", config.GMAIL_SCOPES
            )
...
def send_email(username: str, subject: str, body: str) -> str:
    """Send an email to a user's registered email address."""
    # Get the user's email
    recipient_email = User.get_email(username)
    if not recipient_email:
        return "User not found or no email address stored."
...
def check_eligibility(username: str) -> str:
    """Check if a user is eligible for a coverage increase."""
    current_coverage = Policy.get_coverage(username)
...
def update_policy(username: str) -> str:
    """Update a user's policy to maximum coverage."""
    new_coverage = 50000
    new_premium = 120.0

    success = Policy.update_coverage(username, new_coverage, new_premium)
    if not success:
        return "Failed to update policy. Please try again."

    # Try to send email confirmation
    try:
        email_subject = "Policy Update Confirmation"
        email_body = f"Your coverage has been increased to ${new_coverage}. New monthly premium: ${new_premium:.2f}."
        send_email(username, email_subject, email_body)
    except Exception as e:
...
from database.db_utils import format_ticket_results


def create_ticket(username: str, summary: str, description: str) -> str:
    """Create a new support ticket."""
...
def search_tickets(username: str, query: str = "") -> str:
    """Search for tickets matching a query."""
    results = Ticket.search(username, query)
    return format_ticket_results(results)
...
def stop_recording() -> Dict[str, Any]:
    ...
    if not audio_buffer:
        return {"error": "No audio recorded"}, 400

    # Convert raw audio data to AudioData format
    audio_data = sr.AudioData(
        b"".join(audio_buffer), config.AUDIO_RATE, 2  # Sample width (2 bytes = 16-bit)
    )
