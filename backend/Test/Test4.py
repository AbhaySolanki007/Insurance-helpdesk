...
def update_policy(input_data):
    ...
    try:
        # Directly pass parameters instead of EmailSendInput
        send_email(
            username,
            "Policy Update Confirmation",
            f"Your coverage has been increased to ${new_coverage}. New monthly premium: ${new_premium:.2f}.",
        )
    except Exception as e:
        ...
        try:
            ...
            if isinstance(input_data, str):
                ...
                try:
                    ...
                    # For simple username-only models, allow direct string input
                    if (
                        len(schema_class.model_fields) == 1
                        and "username" in schema_class.model_fields
                    ):
    ...
    return [
        ...
        Tool(
            ...
            func=create_tool_wrapper(
                lambda x: check_eligibility(x), EligibilityCheckInput
            ),
            description="Check if a user is eligible for coverage increase",
        ),
        Tool(
            name="update_policy",
            func=create_tool_wrapper(lambda x: update_policy(x), PolicyUpdateInput),
