...
│   └── db_utils.py         # Database utility functions-NO LONGER REQURIRED
│
├── services/
│   ├── __init__.py
│   ├── auth_service.py     # Authentication services
│   ├── email_service.py    # Email functionality
│   ├── policy_service.py   # Policy management- NO LONGER REQURIRED
...
class UnifiedSupportChain:
    def __init__(
        ...
        self.user_memories = {}  # This will map user_id to ConversationBufferMemory
...
def create_tool_wrapper(
    ...
    """
    Create a wrapper function that handles input parsing and validation for tools.
        ...
        try:
            ...
            if isinstance(input_data, str):
                ...
                try:
                    ...
                    # For simple user_id-only models, allow direct string input
                    if (
                        ...
                        return f"Invalid input format. Expected JSON with fields: {list(schema_class.model_fields.keys())}"
            else:
            ...
            # Call the original function with the validated input
            return func(validated_input)

        except Exception as e:
            # Provide a helpful error message with expected format
...
def create_tools(support_chain):
    ...
    return [
        ...
        Tool(
            ...
            description="Use this ONLY AFTER you have collected a summary and a detailed description from the user. Do not use this tool before asking the user for details first. This tool creates a support ticket in JIRA.",
        ),
        Tool(
            name="search_ticket",
        ...
        Tool(
            name="send_email",
            func=create_tool_wrapper(lambda x: send_email(x), EmailSendInput),
            description="Send an email to the user's registered email address",
        ),
        Tool(
            name="get_user_data",
            func=create_tool_wrapper(lambda x: get_user_data(x), UserDataInput),
            description="Fetch user personal information from the database.",
        ),
        Tool(
            name="get_policy_data",
            func=create_tool_wrapper(lambda x: get_policy_data(x), PolicyDataInput),
