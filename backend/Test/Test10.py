...
# Manually create a persistent connection to the SQLite database.
...
def chat():
    ...
    user_id = data.get("user_id")  # We will use this as the conversation's unique ID
    language = data.get("language", "en")

    if not query or not user_id:
        return jsonify({"error": "Missing 'query' or 'user_id'."}), 400

    # 1. DEFINE the unique ID for the conversation thread.
...
Your primary goal is to handle complex issues by gathering detailed information and creating a support ticket if necessary.
...
2.  **Be Conversational:** Talk to the user to understand their problem fully. Ask follow-up questions until you have enough information.
3.  **Confirm Before Acting:** Once you have the details for a ticket (a clear `summary` and `description`), you MUST confirm them with the user before using the `create_ticket` tool.
...
def l1_node(state: AgentState, agent_executor):
    ...
    # 👇 MODIFY the return statement to include the new decision.
    return {
        "history": [turn_data],
        "is_l2_session": False,
